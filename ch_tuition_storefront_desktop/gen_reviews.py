...
def generate_html(is_code_html):
    html = '<div class="flex overflow-x-auto snap-x snap-mandatory gap-6 pb-6 hide-scrollbar">\n'
    for i, review in enumerate(reviews):
        delay = (i % 4 + 1) * 100
        aos_attr = f' data-aos="fade-up" data-aos-delay="{delay}"' if is_code_html else ''
        card_class = 'bg-surface border border-outline-variant' if is_code_html else 'bg-surface-container-lowest border border-outline-variant'
        text_color_author = 'text-on-primary' if review['bg'] in ['bg-primary', 'bg-[#465f88]'] else 'text-on-secondary'
        
        html += f'''    <div class="snap-center shrink-0 w-[85vw] md:w-[400px] {card_class} p-8 rounded-lg shadow-sm flex flex-col justify-between"{aos_attr}>
        <div>
        ...
        <div class="flex items-center gap-3 mt-auto pt-4 border-t border-outline-variant/30">
            <div class="w-10 h-10 rounded-full {review['bg']} flex items-center justify-center {text_color_author} font-bold">{review['initials']}</div>
            <div>
...
def update_file(filename, is_code_html):
    with open(filename, 'r', encoding='utf-8') as f:
        content = f.read()
    
    new_html = generate_html(is_code_html)
    
    # We replace the grid section.
    pattern = r'<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">.*?</section>'
...
update_file('product.html', False)
update_file('code.html', True)
